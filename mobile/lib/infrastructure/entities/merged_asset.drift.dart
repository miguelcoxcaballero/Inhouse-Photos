...
class MergedAssetDrift extends i1.ModularAccessor {
  ...
  i0.Selectable<MergedAssetResult> mergedAsset({
    ...
    return customSelect(
      'SELECT rae.id AS remote_id, COALESCE((SELECT lae.id FROM local_asset_entity AS lae WHERE lae.checksum = rae.checksum LIMIT 1), (SELECT lae.id FROM remote_asset_cloud_id_entity AS raci INNER JOIN local_asset_entity AS lae ON lae.checksum = raci.cloud_id WHERE raci.asset_id = rae.id LIMIT 1)) AS local_id, rae.name, rae.type, rae.created_at AS created_at, COALESCE(rae.local_date_time, STRFTIME(\'%Y-%m-%dT%H:%M:%fZ\', rae.created_at, \'localtime\')) AS timeline_at, COALESCE(STRFTIME(\'%Y-%m-%d\', rae.local_date_time), STRFTIME(\'%Y-%m-%d\', rae.created_at, \'localtime\')) AS timeline_day, rae.updated_at, rae.width, rae.height, rae.duration_ms, rae.is_favorite, rae.thumb_hash, rae.checksum, rae.owner_id, rae.live_photo_video_id, 0 AS orientation, rae.stack_id, NULL AS i_cloud_id, NULL AS latitude, NULL AS longitude, NULL AS adjustmentTime, rae.is_edited, 0 AS playback_style, rae.uploaded_at FROM remote_asset_entity AS rae LEFT JOIN stack_entity AS se ON rae.stack_id = se.id WHERE rae.deleted_at IS NULL AND rae.visibility = 0 AND rae.owner_id IN ($expandeduserIds) AND(rae.stack_id IS NULL OR rae.id = se.primary_asset_id)UNION ALL SELECT NULL AS remote_id, lae.id AS local_id, lae.name, lae.type, lae.created_at AS created_at, COALESCE(lae.local_date_time, STRFTIME(\'%Y-%m-%dT%H:%M:%fZ\', lae.created_at, \'localtime\')) AS timeline_at, COALESCE(STRFTIME(\'%Y-%m-%d\', lae.local_date_time), STRFTIME(\'%Y-%m-%d\', lae.created_at, \'localtime\')) AS timeline_day, lae.updated_at, lae.width, lae.height, lae.duration_ms, lae.is_favorite, NULL AS thumb_hash, lae.checksum, NULL AS owner_id, NULL AS live_photo_video_id, lae.orientation, NULL AS stack_id, lae.i_cloud_id, lae.latitude, lae.longitude, lae.adjustment_time, 0 AS is_edited, lae.playback_style, NULL AS uploaded_at FROM local_asset_entity AS lae WHERE NOT EXISTS (SELECT 1 FROM remote_asset_entity AS rae WHERE rae.owner_id IN ($expandeduserIds) AND rae.checksum = lae.checksum) AND NOT EXISTS (SELECT 1 FROM remote_asset_cloud_id_entity AS raci INNER JOIN remote_asset_entity AS rae ON rae.id = raci.asset_id WHERE raci.cloud_id = lae.checksum AND rae.owner_id IN ($expandeduserIds)) AND EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 0) AND NOT EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 2) ORDER BY timeline_day DESC, created_at DESC ${generatedlimit.sql}',
      variables: [
      ...
      (i0.QueryRow row) => MergedAssetResult(
        ...
        timelineAt: row.read<DateTime>('timeline_at'),
        timelineDay: row.read<String>('timeline_day'),
  ...
  i0.Selectable<MergedBucketResult> mergedBucket({
    ...
    $arrayStartIndex += userIds.length;
    return customSelect(
      'SELECT COUNT(*) AS asset_count, SUM(is_remote) AS remote_asset_count, bucket_date FROM (SELECT 1 AS is_remote, CASE WHEN ?1 = 0 THEN COALESCE(STRFTIME(\'%Y-%m-%d\', rae.local_date_time), STRFTIME(\'%Y-%m-%d\', rae.created_at, \'localtime\')) WHEN ?1 = 1 THEN COALESCE(STRFTIME(\'%Y-%m\', rae.local_date_time), STRFTIME(\'%Y-%m\', rae.created_at, \'localtime\')) END AS bucket_date FROM remote_asset_entity AS rae LEFT JOIN stack_entity AS se ON rae.stack_id = se.id WHERE rae.deleted_at IS NULL AND rae.visibility = 0 AND rae.owner_id IN ($expandeduserIds) AND(rae.stack_id IS NULL OR rae.id = se.primary_asset_id)UNION ALL SELECT 0 AS is_remote, CASE WHEN ?1 = 0 THEN COALESCE(STRFTIME(\'%Y-%m-%d\', lae.local_date_time), STRFTIME(\'%Y-%m-%d\', lae.created_at, \'localtime\')) WHEN ?1 = 1 THEN COALESCE(STRFTIME(\'%Y-%m\', lae.local_date_time), STRFTIME(\'%Y-%m\', lae.created_at, \'localtime\')) END AS bucket_date FROM local_asset_entity AS lae WHERE NOT EXISTS (SELECT 1 FROM remote_asset_entity AS rae WHERE rae.owner_id IN ($expandeduserIds) AND rae.checksum = lae.checksum) AND NOT EXISTS (SELECT 1 FROM remote_asset_cloud_id_entity AS raci INNER JOIN remote_asset_entity AS rae ON rae.id = raci.asset_id WHERE raci.cloud_id = lae.checksum AND rae.owner_id IN ($expandeduserIds)) AND EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 0) AND NOT EXISTS (SELECT 1 FROM local_album_asset_entity AS laa INNER JOIN local_album_entity AS la ON laa.album_id = la.id WHERE laa.asset_id = lae.id AND la.backup_selection = 2)) GROUP BY bucket_date ORDER BY bucket_date DESC',
      variables: [
      ...
      (i0.QueryRow row) => MergedBucketResult(
        ...
        remoteAssetCount: row.readNullable<int>('remote_asset_count'),
  ...
  i6.$RemoteAssetCloudIdEntityTable get remoteAssetCloudIdEntity =>
      i1.ReadDatabaseContainer(
        attachedDatabase,
      ).resultSet<i6.$RemoteAssetCloudIdEntityTable>(
...
class MergedBucketResult {
  ...
  final int? remoteAssetCount;
  final String bucketDate;
  MergedBucketResult({
    required this.assetCount,
    this.remoteAssetCount,
